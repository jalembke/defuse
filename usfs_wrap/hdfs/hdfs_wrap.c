...
struct namenode_state nn_state;
...
static
int hadoop_fuse_write(const char * src, const char * data, size_t len, off_t offset, uint64_t * fi);

static
int hadoop_fuse_ftruncate(const char * path, off_t offset, uint64_t * fi);

static
int hadoop_fuse_read(const char * src, char * buf, size_t size, off_t offset, uint64_t * fi);
...
{
  return (struct namenode_state *) &nn_state;
}
...
static
int hadoop_fuse_fsync(const char * src, int datasync, uint64_t * fi)
{
  (void) datasync;
  (void) fi;
  int res;
...
static
int hadoop_fuse_mknod(const char * src, mode_t perm)
{
  return hadoop_fuse_createemptyfile(src, perm, NULL);
}

static
int hadoop_fuse_open(const char * path, int flags, uint64_t* fi)
{
  int res;
...
int hadoop_fuse_ftruncate(const char * path, off_t offset, uint64_t* fi)
{
  ...
  struct Hadoop_Fuse_FileHandle * fh = (struct Hadoop_Fuse_FileHandle *) fi;
  uint64_t oldlength;
...
void usfs_init(const char* mount_path, const char* backend_path)
{
  ...
  const char* host = strtok((char*)backend_path, ":");
  uint16_t port = atoi(strtok(NULL, ":"));

  memset(&nn_state, 0, sizeof(nn_state));
  nn_state.clientname = "hadoop_fuse";
  pthread_mutex_init(&nn_state.connection.mutex, NULL);

  res = hadoop_rpc_connect_namenode(&nn_state, host, port);
  if(res < 0)
  {
...
#ifndef NDEBUG
  openlog (nn_state.clientname, LOG_CONS | LOG_PID | LOG_NDELAY | LOG_PERROR, LOG_USER);
  ...
  syslog(
    ...
    nn_state.packetsize,
    nn_state.blocksize,
    nn_state.replication,
    nn_state.bytesperchecksum,
    nn_state.checksumtype);
#endif
