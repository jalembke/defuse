...
static inline struct sshfs_file *get_sshfs_file(uint64_t *fi)
{
	return (struct sshfs_file *) (uintptr_t) fi;
}

static int sshfs_chmod(const char *path, mode_t mode, uint64_t *fi)
{
	(void) fi;
	int err;
	struct buffer buf;
...
static int sshfs_open_common(const char *path, int flags, mode_t mode, uint64_t *fi)
{
	int err;
	int err2;
	struct buffer buf;
...
static int sshfs_fsync(int isdatasync, uint64_t *fi)
{
	int err;
	(void) isdatasync;
...
static int sshfs_release(const char *path, uint64_t *fi)
{
	struct sshfs_file *sf = get_sshfs_file(fi);
...
static int sshfs_read(char *rbuf, size_t size, off_t offset, uint64_t *fi)
{
	struct sshfs_file *sf = get_sshfs_file(fi);
...
static int sshfs_write(const char *wbuf, size_t size, off_t offset, uint64_t *fi)
{
	int err;
	struct sshfs_file *sf = get_sshfs_file(fi);
...
static int sshfs_create(const char *path, int flags, mode_t mode, uint64_t *fi)
{
	if (sshfs.createmode_workaround)
		mode = 0;

	return sshfs_open_common(path, flags, mode, fi);
}

static int sshfs_truncate(const char *path, off_t size, uint64_t *fi)
{
	int err;
	struct buffer buf;
	struct sshfs_file *sf = NULL;
...
static int sshfs_getattr(const char *path, struct stat *stbuf, uint64_t *fi)
{
	int err;
	struct buffer buf;
	struct buffer outbuf;
...
static int sshfs_access(const char *path, int mask)
{
	...
	if (mask & X_OK) {
		err = sshfs_getattr(path, &stbuf, NULL);
		if (!err) {
...
static struct sshfs_operations sshfs_oper = {
		.read       = sshfs_read,
		.write      = sshfs_write,
		.fsync      = sshfs_fsync,
...
int usfs_init(const char* mount_path, const char* backend_path)
{
	int res;
	char *tmp;
	char *fsname;
	...
	sshfs.passive = 0;
	sshfs.detect_uid = 0;
	sshfs.mountpoint = strdup(mount_path);
	sshfs.host = strdup(backend_path);
	sshfs.sync_read = 0;
...
{
	int rc = sshfs.op->read(buf, size, offset, &fh);
	return -rc;
}
...
{
	int rc = sshfs.op->truncate(path, length, NULL);
	return -rc;
}

int usfs_ftruncate(uint64_t fh, off_t offset)
{
	int rc = sshfs.op->truncate(NULL, offset, &fh);
	return -rc;
}

int usfs_fgetattr(uint64_t fh, struct stat *stbuf)
{
	int rc = sshfs.op->getattr(NULL, stbuf, &fh);
	return -rc;
}

int usfs_getattr(const char* path, struct stat *stbuf, int flags)
{
	int rc = sshfs.op->getattr(path, stbuf, NULL);
	return -rc;
}

int usfs_unlink(const char* path)
{
	int rc = sshfs.op->unlink(path);
	return -rc;
}

int usfs_close(uint64_t ret_fh)
{
	int rc = sshfs.op->release(NULL, &ret_fh);
	return -rc;
}
